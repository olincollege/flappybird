...
Test(bird_tests, bird_initializes_correctly) {
    ...
    cr_expect_eq(bird.x, BIRD_X_POS, "Bird x position should be initialized to BIRD_X_POS");
    cr_expect_float_eq(bird.y, BIRD_Y_POS, 0.001, "Bird y position should be initialized to BIRD_Y_POS");
    cr_expect_eq(bird.width, BIRD_WIDTH, "Bird width should be initialized to BIRD_WIDTH");
    cr_expect_eq(bird.height, BIRD_HEIGHT, "Bird height should be initialized to BIRD_HEIGHT");
    cr_expect_float_eq(bird.y_vel, BIRD_VEL, 0.001, "Bird vertical velocity should be initialized to BIRD_VEL");
    cr_expect_eq(bird.jumpBool, FALSE, "Bird jumpBool should be initialized to FALSE");
}
...
Test(bird_tests, bird_resets_correctly_after_end_game) {
    ...
    cr_assert_float_eq(bird.y, BIRD_Y_POS, 0.001, "Bird y position did not reset correctly");
    ...
    cr_assert_float_eq(bird.y_vel, BIRD_VEL, 0.001, "Bird vertical velocity did not reset correctly");
    cr_assert_eq(bird.jumpBool, FALSE, "Bird jumpBool did not reset correctly after end game");
}
